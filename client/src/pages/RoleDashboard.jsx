...
import { completePilot, getRoleWorkspace, startPilot, verifyPilot } from '../services/roleService';
...
export default function RoleDashboard() {
  ...
  const handleVerify = async (pilot) => {
    const approved = window.confirm('Approve this pilot verification? Choose Cancel to reject it.');
    setActionId(pilot._id);
    try {
      await verifyPilot(pilot._id, { status: approved ? 'Approved' : 'Rejected' });
      await loadWorkspace();
    } catch (requestError) { setError(requestError.response?.data?.message || 'Could not update verification'); }
    finally { setActionId(''); }
  };
